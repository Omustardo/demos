...
func (keyEventList *glfwKeyEventList) freeze() []glfwKeyEvent {
	...
	frozen := *keyEventList
	*keyEventList = make([]glfwKeyEvent, 0, eventListCap) // Note that the warning in Intellij about assigning to method receiver is wrong. It doesn't realize that we're using a pointer.
	return frozen
}
